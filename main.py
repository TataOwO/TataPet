...
class Object(tk.Tk):
    ...
    def Mouse_moving(self, event):
        if not self.canMove:
            ...
            moveY = event.y - self.y
            x = self.window.winfo_x() + moveX
            y = self.window.winfo_y() + moveY
            self.moveFrame.pop(0)
            self.moveFrame.append([moveX,moveY])
            if x < 0:
                x = 0
    ...
    def processMove(self):
        ...
        x, y = int(x), int(y)
        self.window.geometry(f"+{x}+{y}")
        self.window.after(30,self.processMove)
        


if __name__ == "__main__": # 1280, 720  1215, 620
    Tata = Object(file_name, True, True)
    Tata.window.after(30,Tata.processMove)
